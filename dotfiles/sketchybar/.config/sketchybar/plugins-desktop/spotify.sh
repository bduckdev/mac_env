...
update_track() {
	...
	if [[ -z $SPOTIFY_JSON ]]; then
		sketchybar --set $NAME background.color=0xffffbd6e label.drawing=no
		return
	fi
	...
	if [ $PLAYER_STATE = "Playing" ]; then
		...
		sketchybar --set $NAME label="${ARTIST} - ${TRACK}" label.drawing=yes background.color=0xff5eff6c

	elif [ $PLAYER_STATE = "Paused" ]; then
		sketchybar --set $NAME background.color=0xffffbd6e
